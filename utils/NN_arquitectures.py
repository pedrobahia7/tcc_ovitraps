...
def compute_weights_slow_decay(values, highlight_targets):
    ...
    for value in values:
        ...
        highlight_factor = sum(
            1 / (abs(value - target) * 0.4 + 1)
...
class ParetoLoss(nn.Module):
    ...
    def forward(self, y, alpha, x_m=1):
        ...
        for i in range(y.shape[0]):
            loss += torch.log(alpha[i]) * x_m - (alpha[i] + 1) * torch.sum(
                torch.log(y[i])
            )
...
class NeuralNetworkExponential(nn.Module):
    ...
    def forward(self, x):
        ...
        lamb = self.softplus(
            self.exp(torch.cat((out3, logit), dim=1))
        )  # concat weights and logits. Relu to avoid negative values
        return logit, lamb
...
class NeuralNetworkPareto(nn.Module):
    ...
    def forward(self, x):
        ...
        alpha = self.pareto_layer(
            torch.cat((out3, logit), dim=1)
        )  # concat weights and logits. To use class instead, change to torch.cat((out3, logit.argmax(1)), dim=1) and self.exp
        return logit, alpha
...
class LogisticRegression(nn.Module):
    def __init__(self, model_input, input_3d, model_type):
        ...
        if self.input_3d:
            depth = (
                4  # Fixed depth as novos + latitude +longitude  + distance
            )
            self.conv1 = nn.Conv1d(
                in_channels=depth,
                out_channels=1,
