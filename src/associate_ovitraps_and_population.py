...
def resolve_paths(params: dict) -> dict:
    """Resolve all required paths from params."""
    data = params["all"]["paths"]["data"]
    return {
        "geojson": Path(
            data["processed"]["census_equivalence"]["sectors_geojson"]
        ),
        "ovitraps": Path(data["processed"]["ovitraps"]),
        "centroids": Path(data["processed"]["centroids"]),
        "centroids_idw": Path(data["processed"]["centroids_idw"]),
    }
...
def calculate_sector_centroids(
    ...
    available_columns = [
        col for col in output_columns if col in gdf.columns
    ]
    centroids_df = gdf[available_columns].copy()

    # Sort by sector code
    centroids_df = centroids_df.sort_values("CD_SETOR").reset_index(
        drop=True
    )
    ...
    if output_path is None:
        ...
        output_path = (
            input_path.parent / f"{input_path.stem}_centroids.csv"
        )
    ...
    assert "centroid_latitude" in centroids_df.columns, (
        "Missing centroid_latitude"
    )
    assert "centroid_longitude" in centroids_df.columns, (
        "Missing centroid_longitude"
    )
    assert centroids_df["centroid_latitude"].notnull().all(), (
        "Latitude contains null values"
    )
    assert centroids_df["centroid_longitude"].notnull().all(), (
        "Longitude contains null values"
    )
...
def calculate_idw_for_centroids(
    ...
    # Input validation
    assert isinstance(centroids_df, pd.DataFrame), (
        "centroids_df must be a DataFrame"
    )
    assert "centroid_latitude" in centroids_df.columns, (
        "Missing centroid_latitude"
    )
    ...
    eggs["week_num"] = (
        eggs["epidemic_date"].str.split("W").str[1].astype(int)
    )
    ...
    eggs["biweek"] = (
        eggs["epi_year"]
        + "W"
        + eggs["biweek_num"].astype(str).str.zfill(2)
    )
    ...
    print(
        f"Calculating IDW for {len(centroids_df)} centroids across {len(biweeks)} biweeks..."
    )
    ...
    for idx, biweek in enumerate(biweeks):
        ...
        if len(trap_coords) <= n_neighbors:
            ...
            nearest_indices_all = np.tile(
                np.arange(len(trap_coords)), (len(centroids_df), 1)
            )
        else:
            # Get k nearest neighbors for each centroid (vectorized)
            nearest_indices_all = np.argpartition(
                distances, n_neighbors, axis=1
            )[:, :n_neighbors]
        ...
        for i in range(len(centroids_df)):
            ...
            if np.any(nearest_distances == 0):
                ...
                idw_value = np.sum(weights * nearest_values) / np.sum(
                    weights
                )
            ...
            results.append(
                {
                    ...
                    "narmads_used": ",".join(
                        str(n) for n in nearest_narmads
                    ),
                }
            )
    ...
    idw_df = idw_df.sort_values(["CD_SETOR", "biweek"]).reset_index(
        drop=True
    )
    ...
    print(
        f"IDW egg value range: [{idw_df['idw_egg_value'].min():.2f}, "
    ...
    assert idw_df["idw_egg_value"].notnull().all(), (
        "IDW values contain nulls"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Associate ovitraps with population sectors via centroids and IDW."
    )
    parser.add_argument("--params-path", default=str(DEFAULT_PARAMS_PATH))
    parser.add_argument("--geojson-path", default=None)
    parser.add_argument("--ovitraps-path", default=None)
    parser.add_argument("--centroids-output", default=None)
    parser.add_argument("--idw-output", default=None)
...
def main() -> None:
    """Run the centroids + IDW calculation pipeline."""
    args = parse_args()
    params = load_params(args.params_path)
    paths = resolve_paths(params)

    geojson_path = args.geojson_path or str(paths["geojson"])
    ovitraps_path = args.ovitraps_path or str(paths["ovitraps"])
    centroids_output = args.centroids_output or str(paths["centroids"])
    idw_output = args.idw_output or str(paths["centroids_idw"])

    print("Step 1: Calculating sector centroids...")
    centroids_df = calculate_sector_centroids(
        geojson_path=geojson_path,
        output_path=centroids_output,
    )
    ...
    idw_df = calculate_idw_for_centroids(
        ...
        power=args.power,
        output_path=idw_output,
    )
    # Save to CSV
    print(f"\nSaving IDW results to: {idw_output}")
    idw_df.to_csv(idw_output, index=False)
