...
def cluster_cases_by_year(dengue_data, grid_points, year):
    ...
    if len(year_data) == 0:
        return pd.DataFrame(
            columns=["latitude", "longitude", "cases", "cluster_id"]
        )

    # Find nearest grid point for each case
    cluster_cases = generic.nearest_neighbors(
        year_data[["latitude", "longitude"]].values, grid_points.values
    )
    refpoint_idx, cases_per_refpoint = np.unique(
        cluster_cases, return_counts=True
    )
...
dengue_filtered = dengue_data[
    ["latitude", "longitude", "anoepid"]
].dropna()
...
marks = [
    "*" if year in project_utils.EPIDEMY_YEARS else ""
...
def create_year_comparison_data():
    ...
    for year in available_years:
        year_clusters = cluster_cases_by_year(
            dengue_filtered, grid_points, year
        )
        year_clusters[f"cases_{year}"] = year_clusters["cases"]
        all_data.append(
            year_clusters[
                ["latitude", "longitude", "cluster_id", f"cases_{year}"]
            ]
        )
...
def base_map_figure(center_lat=None, center_lon=None, zoom=10):
    ...
    fig.add_trace(
        go.Scattermapbox(lat=[], lon=[], mode="markers", showlegend=False)
    )
    return fig
    ...
    [
            ...
            [
                    ...
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                ...
                                for year, mark in zip(
                                    available_years, marks
                                )
                            ],
                            value="cases_2015_16",
                            style={"fontSize": "11px"},
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            f"Current: {spacing_m}m",
                            id="current-spacing-display",
        ...
        html.Div(
            [
                # Two maps - full width
                html.H3(
                    "Geographical distribution of dengue cases by year"
                ),
                html.Div(
                    [
                        html.Div(
                ...
                html.Div(
                    [
                        html.H3(
                            "Ratio Comparison Map (First Year / Second Year)"
                        ),
                        html.Div(
                            [
                                html.Label(
                        ...
                        html.Div(
                            [
                                dcc.Checklist(
                                    ...
                                    options=[
                                        {
                                            "label": " Use normalized values (z-score)",
                                            "value": "relative",
                                        }
                                    ],
                        ...
                        dcc.Graph(
                            id="map-ratio", style={"height": "900px"}
                        ),
                    ],
                ...
                html.Div(
                    [
                        html.Button(
                            "Update map with selected points",
                            id="update-map-btn",
                            style={
                                "marginRight": "10px",
...
def update_grid_data(n_clicks, new_spacing, current_spacing):
    ...
    def create_year_comparison_data_new_grid():
        ...
        for year in available_years:
            year_clusters = cluster_cases_by_year(
                dengue_filtered, new_grid_points, year
            )
            year_clusters[f"cases_{year}"] = year_clusters["cases"]
            all_data.append(
                year_clusters[
        ...
        for year in available_years:
            merged_df[f"cases_{year}"] = merged_df[f"cases_{year}"].fillna(
                0
            )
...
def update_scatter(xcol, ycol, grid_data, log_x_checked, log_y_checked):
    ...
    if use_log_x or use_log_y:
        ...
        mask = np.ones(
            len(x_vals), dtype=bool
        )  # Include all points for linear scales
...
def store_selected_points(selectedData, current_selected, grid_data):
    ...
    for p in selectedData["points"]:
        ...
        if point_key in current_set:
            ...
            current_selected = [
                pt
                for pt in current_selected
                if not (
                    round(pt["lat"], 6) == lat_round
                    and round(pt["lon"], 6) == lon_round
                )
            ]
...
def update_map(_, selected_points, grid_data, series_x, series_y):
    ...
    fig = px.scatter_mapbox(
        dff, lat="lat", lon="lon", custom_data=["cases_x", "cases_y"]
    )
...
def update_series_maps(
    series_x, series_y, grid_data, current_spacing, min_cases, use_relative
):
    ...
    max_cases_1 = (
        data_series1[series_x].max() if len(data_series1) > 0 else 1
    )
    max_cases_2 = (
        data_series2[series_y].max() if len(data_series2) > 0 else 1
    )
    ...
    fig1.update_traces(
        hovertemplate="<b>Counts: %{customdata[0]}</b><extra></extra>"
    )
    fig2.update_traces(
        hovertemplate="<b>Counts: %{customdata[0]}</b><extra></extra>"
    )
    ...
    fig1.update_traces(
        marker=dict(
            size=np.sqrt(data_series1[series_x])
            * grid_scale,  # Size varies with case count
            opacity=0.8,
        )
    )
    ...
    fig2.update_traces(
        marker=dict(
            size=np.sqrt(data_series2[series_y])
            * grid_scale,  # Size varies with case count
            opacity=0.8,
        )
    )
    ...
    if is_relative:
        ...
        data_ratio["zscore_x"] = (
            np.nan
        )  # Not applicable for absolute values
    ...
    data_ratio = data_ratio[
        (data_to_plot[series_x] >= min_cases)
        & (data_to_plot[series_y] >= min_cases)
    ].copy()
    ...
    fig_ratio = px.scatter_mapbox(
        ...
        range_color=[
            0,
            data_ratio["ratio"].quantile(0.95),
        ],  # Scale from 0 to 95th percentile to handle outliers
    )
    ...
    fig_ratio.update_traces(
        marker=dict(size=np.sqrt(total_cases) * grid_scale, opacity=0.8)
    )
